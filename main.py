...
def index_univer():
    ...
    subject = db.sqlalchemy_session.query(Univer).all()

    return render_template('univer.html', univer=univer)


@app.route('/new_univedr', methods=['GET', 'POST'])
def new_univer():
    form = UniverForm()

    if request.method == 'POST':
        if not form.validate():
            return render_template('univer_form.html', form=form, form_name="New univer",
                                   action="new_univer")
        else:
            subject_obj = Univer(
                name=form.name.data,
    ...
    return render_template('univer_form.html', form=form, formname="New univer", action="new_univer")
...
def edit_univer():
    ...
    if request.method == 'GET':
        ...
        if not form.validate():
            return render_template('univer_form.html', form=form, form_name="Edit univer",
                                   action="edit_univer")
        else:
            db = PostgresDb()
            # find discipline
...
def delete_univer():
    ...
    result = db.sqlalchemy_session.query(Univer).filter(
        Univer.addr == addr,
        Univer.name == name).one()
